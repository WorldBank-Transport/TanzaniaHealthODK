...
package tz.health.collect.android.external;
            ...
            for (Map.Entry<String, String> paramEntry : exParams.entrySet()) {
                ...
                try {
                    ...
                    if (paramEntryValue.startsWith("'")) {
                        ...
                        Cursor c = Collect.getInstance().getContentResolver().query(InstanceProviderAPI.InstanceColumns.CONTENT_URI, null, selection, selectionArgs, null);
                        if (c != null && c.getCount() > 0) {
                            // should only ever be one
                            c.moveToFirst();
                            instanceProviderID = c.getString(c.getColumnIndex(InstanceProviderAPI.InstanceColumns._ID));
                        }
                        if (c != null) {
                            c.close();
                        }
