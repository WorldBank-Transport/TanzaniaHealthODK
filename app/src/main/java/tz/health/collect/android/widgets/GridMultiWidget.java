...
package tz.health.collect.android.widgets;
...
import tz.health.collect.android.external.ExternalDataUtil;
import tz.health.collect.android.external.ExternalSelectChoice;
import tz.health.collect.android.utilities.FileUtils;
import tz.health.collect.android.views.AudioButton;
import tz.health.collect.android.views.ExpandedHeightGridView;
...
public class GridMultiWidget extends QuestionWidget {
    ...
    public GridMultiWidget(Context context, FormEntryPrompt prompt, int numColumns) {
        ...
        for (int i = 0; i < mItems.size(); i++) {
            ...
            if ( audioURI != null) {
            	audioHandlers[i] = new AudioButton.AudioHandler(prompt.getIndex(), sc.getValue(), audioURI);
            } else {
