...
public class InstanceProvider extends ContentProvider {
        ...
        public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        	...
        	if ( oldVersion == 1 ) {
        		...
        		db.execSQL("UPDATE " + INSTANCES_TABLE_NAME + " SET " +
        					InstanceProviderAPI.InstanceColumns.CAN_EDIT_WHEN_COMPLETE + " = '" + Boolean.toString(true) + "' WHERE " +
        					InstanceProviderAPI.InstanceColumns.STATUS + " IS NOT NULL AND " +
    ...
    public Uri insert(Uri uri, ContentValues initialValues) {
        ...
        if (values.containsKey(InstanceProviderAPI.InstanceColumns.LAST_STATUS_CHANGE_DATE) == false) {
            values.put(InstanceProviderAPI.InstanceColumns.LAST_STATUS_CHANGE_DATE, now);
        }

        if (values.containsKey(InstanceProviderAPI.InstanceColumns.DISPLAY_SUBTEXT) == false) {
            Date today = new Date();
        ...
        if (values.containsKey(InstanceProviderAPI.InstanceColumns.STATUS) == false) {
